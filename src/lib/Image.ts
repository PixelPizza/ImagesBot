...
class ImageData {
	...
	public deserialize() {
		return this.container.deserialize(this.data, { fromEncoding: "binary", toEncoding: "base64" });
	}
}
...
export class Image {
	...
	public set data(data: string | ImageData) {
		...
		this.next = [];
		this.#data = new ImageData({ container: this.container, data: data.toString() });
	}
	...
	public constructor(public readonly options: ImageOptions) {
		...
		this.#data = new ImageData({ container: this.container, data: options.data });
		this.previous = (options.previous ?? []).map((data) => new ImageData({ container: this.container, data }));
		this.next = (options.next ?? []).map((data) => new ImageData({ container: this.container, data }));
	}
